...
from transformers.data.data_collator import default_data_collator as torch_default_data_collator
...
def tokenize(
    ...
    dataset = Dataset.from_csv(
        data_dir, 
        sep='\t',
        features=Features({'Chr': Value('string'),
        'Start': Value('int64'),
        'End': Value('int64'),
        'Type': Value('string'),
...
def train(
    ...
    """Train a supervised PlantCAD2 model without loading the weights.
    ...
    """
    logger.info(f"Loading base model from {model_name}")
    model = load_base_model(model_name, num_labels=num_labels)
...
def evaluate(
    ...
    model = AutoModelForSequenceClassification.from_pretrained(checkpoint_dir, trust_remote_code=True, num_labels=num_labels)
...
def load_base_model(model_name: str, num_labels: int) -> AutoModelForSequenceClassification:
    # Load the configuration for the model architecture
