...
class DNADataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label_vec = np.array([float(c) for c in label_str])
        y = torch.tensor(label_vec, dtype=torch.float32)
        return x, y


class DanQLightning(pl.LightningModule):
    def __init__(self, learning_rate=0.01, num_labels=1):
        super().__init__()
    ...
    def training_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            ...
            try:
                ...
                f1 = metrics.f1_score(y_cpu, (y_hat_cpu >= 0.5).astype(int), average='micro')
                roc_auc = roc_auc_score(y_cpu, y_hat_cpu, average='micro')
                average_precision = metrics.average_precision_score(y_cpu, y_hat_cpu, average='micro')
    ...
    def on_validation_epoch_end(self):
        ...
        f1 = metrics.f1_score(y_true, (y_pred >= 0.5).astype(int), average='micro')
        roc_auc = roc_auc_score(y_true, y_pred, average='micro')
        average_precision = metrics.average_precision_score(y_true, y_pred, average='micro')
...
def train(
    ...
    trainLabel = trainDATA['Label'].apply(lambda x: list(map(float, str(x)))).tolist()
    validLabel = validDATA['Label'].apply(lambda x: list(map(float, str(x)))).tolist()
    ...
    # Initialize model
    model = DanQLightning(learning_rate=learning_rate, num_labels=num_labels)
...
def predict(
    ...
    model.eval()
    model.to(torch.device(device))
    dummy_labels = np.zeros((len(sequences), model.num_labels))
    ...
    results_df.columns = [f"class_{i}" for i in range(results_df.shape[1])]
    results_df.to_csv(output_file, index=False)
