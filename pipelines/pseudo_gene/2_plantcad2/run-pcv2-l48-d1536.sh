...
CUDA_VISIBLE_DEVICES=${DEVICE} python ../main.py -input ${DATA_DIR}${TAXA}_start_sites_filtered.fasta \
    -output ${OUTPUT}${TAXA}_start_sites_mask_ATG_${PREFIX}.h5 \
    -model ${CHECKPOINT} \
    -batchSize ${BATCH_SIZE} -tokenIdx 4094,4095,4096

CUDA_VISIBLE_DEVICES=${DEVICE} python ../main.py -input ${DATA_DIR}${TAXA}_stop_sites_filtered.fasta \
    -output ${OUTPUT}${TAXA}_stop_sites_mask_TAG_${PREFIX}.h5 \
    -model ${CHECKPOINT} \
    -batchSize ${BATCH_SIZE} -tokenIdx 4094,4095,4096

CUDA_VISIBLE_DEVICES=${DEVICE} python ../main.py -input ${DATA_DIR}${TAXA}_donor_filtered.fasta \
    -output ${OUTPUT}${TAXA}_donor_mask_GT_${PREFIX}.h5 \
    -model ${CHECKPOINT} \
    -batchSize ${BATCH_SIZE} -tokenIdx 4095,4096

CUDA_VISIBLE_DEVICES=${DEVICE} python ../main.py -input ${DATA_DIR}${TAXA}_acceptor_filtered.fasta \
