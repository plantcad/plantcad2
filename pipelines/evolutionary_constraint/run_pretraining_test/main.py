...
def parse_args():
    ...
    parser.add_argument("-length", dest="length", type=int, default=8192)
...
if args.length <= 8192:
    ...
    for seq in sequences:
        ...
        if start < 0:
            start = 0
        if start + args.length > len(seq):
            start = len(seq) - args.length
        final_seq = seq[start:start + args.length]
        
        assert len(final_seq) == args.length, f"Got {len(final_seq)} instead of {args.length}"
        
        new_sequences.append(final_seq)
    sequences = new_sequences

names = df['assembly'].tolist()
# Create dataset
