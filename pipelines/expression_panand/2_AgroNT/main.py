...
def tokenize(
    ...
    sequence_length : int, optional
        Expected length of all input sequences, by default 1000
    ...
    if output_path is None:
        output_path = str(Path(data_dir).with_suffix('.agront.parquet'))
    if num_proc is None:
    ...
    tokenized_dataset = dataset.map(
        ...
        remove_columns=["genome", "id", "seqid", "end", "start", "strand", "orthogroup"],
        num_proc=num_proc,
    )
...
def train(
    ...
    task_type : str, optional
        Task type, either "classification" or "regression", by default "classification"
    train_batch_size : int, optional
        Batch size for training, by default
    ...
    """
    logger.info(f"Loading base model from {model_name}")
    base_model = load_base_model(model_name, task_type=task_type)
    ...
    train_dataset = Dataset.from_parquet(
        str(Path(data_dir) / f"train.agront.parquet"),
        keep_in_memory=False,
    )
    eval_dataset = Dataset.from_parquet(
        str(Path(data_dir) / f"valid.agront.parquet"),
        keep_in_memory=False,
    )
