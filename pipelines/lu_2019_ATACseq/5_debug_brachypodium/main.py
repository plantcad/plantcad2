...
def train(
    ...
    all_dataset = Dataset.from_parquet(
        data_dir,
        keep_in_memory=False,
    )

    train_dataset = all_dataset.filter(lambda example: example['chr'] != 'Bd5')
    eval_dataset = all_dataset.filter(lambda example: example['chr'] == 'Bd5')
...
def load_base_model(model_name: str) -> AutoModelForSequenceClassification:
