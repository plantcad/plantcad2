...
CUDA_VISIBLE_DEVICES=1 python main.py train  \
    ...
    --output_dir ${CKPT_DIR} \
    --run_id accessible_angiosperm_c600_cnn-lstm-imbalance-lr-1e-3 \
    --learning_rate 0.001 \
...
UDA_VISIBLE_DEVICES=1 python main.py train  \
    --train $DATA_DIR/train.tsv \
    --valid $DATA_DIR/valid.tsv \
    --output_dir ${CKPT_DIR} \
    --run_id accessible_angiosperm_c600_cnn-lstm-balance-lr-1e-3 \
    --learning_rate 0.001 \
    --wandb_project PlantCAD2 \
    --balance True
