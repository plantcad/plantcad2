...
df = pd.read_csv("/workdir/jz963/datasets/PlantCAD2_tasks/zero_shot/conservation_within_poaceae_non_tis/test.tsv", sep='\t')
REF = df['seq'].str[4095]
...
for ctx in ctx_window:
    for model in models:
        ...
        logitPath = f'../../results/review/5_zero_shot_long_ctx/conservation_within_poaceae_non_tis/{model}-c{ctx}.tsv'
...
# add 8192
new_rows = [
    {'model': 'pcv2-small', 'context': '8192', 'roc_auc': 0.6462},
    {'model': 'pcv2-medium', 'context': '8192', 'roc_auc': 0.6869},
    {'model': 'pcv2-large', 'context': '8192', 'roc_auc': 0.7125}
]
results_df = pd.concat([results_df, pd.DataFrame(new_rows)], ignore_index=True)

# save results_df to csv
results_df.to_csv("conservation_within_poaceae_non_tis_results.tsv", sep='\t', index=False)
...
plt.tight_layout()
plt.savefig('conservation_within_poaceae_nontis.pdf', format='pdf', bbox_inches='tight')
plt.show()