...
df = pd.read_csv("/workdir/jz963/datasets/PlantCAD2_tasks/zero_shot/conservation_within_poaceae_tis/test.tsv", sep='\t')
REF = df['seq'].str[4095]
...
for ctx in ctx_window:
    for model in models:
        ...
        logitPath = f'../../results/review/5_zero_shot_long_ctx/conservation_within_poaceae_tis/{model}-c{ctx}.tsv'
...
# add 8192
new_rows = [
    {'model': 'pcv2-small', 'context': '8192', 'roc_auc': 0.6319},
    {'model': 'pcv2-medium', 'context': '8192', 'roc_auc': 0.6647},
    {'model': 'pcv2-large', 'context': '8192', 'roc_auc': 0.6703}
]
results_df = pd.concat([results_df, pd.DataFrame(new_rows)], ignore_index=True)

# save results_df to csv
results_df.to_csv("conservation_within_poaceae_tis_results.tsv", sep='\t', index=False)
...
plt.tight_layout()
plt.savefig('conservation_within_poaceae_tis.pdf', format='pdf', bbox_inches='tight')
plt.show()