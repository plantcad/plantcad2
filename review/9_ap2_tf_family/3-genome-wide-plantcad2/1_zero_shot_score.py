...
def load_model_and_tokenizer(model_dir, device):
    ...
    if 'gpn' in model_dir:
        logging.info(f"Loading GPN model from {model_dir}")
...
def main():
    ...
    if args.inputVCF is not None:
        ...
        logging.info(f"[Rank {rank}] Starting distributed prediction ({rank + 1}/{world_size})")
        
        # Load reference genome and BED file
        fasta_dict = load_fasta(args.inputFasta)
        bed_df = load_bed_file(args.inputBed)

        # Partition BED file for distributed processing
        total_len = len(bed_df)
        chunk_size = math.ceil(total_len / world_size)
        start_idx = rank * chunk_size
        end_idx = min(start_idx + chunk_size, total_len)
        bed_df = bed_df.iloc[start_idx:end_idx]
        logging.info(f"[Rank {rank}] Processing BED rows {start_idx} to {end_idx} (Total: {total_len})")
        ...
        if not sequences:
            logging.warning(f"[Rank {rank}] No valid sequences extracted. This might be expected if the split is empty.")
            results = []
        else:
        ...
        # Write output to rank-specific file
        temp_output = f"{args.output}.rank{rank}"
        write_llr_output(results, temp_output, args.aggregation, args.outputRawProb)
        logging.info(f"[Rank {rank}] Saved local predictions to {temp_output}")

        if rank == 0:
            logging.info(f"[Rank 0] Waiting for other ranks to finish...")
            while True:
                found = sum(1 for r in range(world_size)
                            if os.path.exists(f"{args.output}.rank{r}"))
                if found == world_size:
                    break
                time.sleep(5)
