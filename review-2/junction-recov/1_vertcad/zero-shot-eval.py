...
class ZeroShotEval:
    def evo_cons(
        ...
        logger.info(f"[Rank {rank}/{world_size}] Starting evo_cons")
        ...
        if logits_path is not None:
            ...
            start_idx, end_idx = partition_indices(len(df), rank, world_size)
            df_local = df.iloc[start_idx:end_idx]
            logger.info(f"[Rank {rank}] Processing rows {start_idx}–{end_idx} ({len(df_local)} sequences)")

            dataset = SingleMaskDataset(df_local[seq_column], tok, token_idx)
            loader = DataLoader(dataset, batch_size=batch_size, shuffle=False, num_workers=1)
            probs_local = _masked_probs(model_, tok, loader, dev, desc=f"[Rank {rank}] Masked logits @ {token_idx}")
            ...
            if world_size == 1:
                ...
                assert save_logits, "save_logits is required for multi-node runs"
                rank_path = Path(save_logits).with_suffix(f".rank{rank}.tsv")
                pd.DataFrame(probs_local, columns=list(NUCLEOTIDES)).to_csv(rank_path, sep="\t", index=False)
                logger.info(f"[Rank {rank}] Saved per-rank logits to {rank_path}")

                if rank != 0:
                    return

                logger.info("[Rank 0] Waiting for all ranks to finish...")
                while True:
                    done = sum(
                        1 for r in range(world_size)
                        if Path(save_logits).with_suffix(f".rank{r}.tsv").exists()
                    )
                    if done == world_size:
                        break
                    time.sleep(5)
                ...
                probs = np.vstack([
                    pd.read_csv(Path(save_logits).with_suffix(f".rank{r}.tsv"), sep="\t").values
                    for r in range(world_size)
                ])
                pd.DataFrame(probs, columns=list(NUCLEOTIDES)).to_csv(save_logits, sep="\t", index=False)
                logger.info(f"[Rank 0] Aggregated logits saved to {save_logits}")
                for r in range(world_size):
                    Path(save_logits).with_suffix(f".rank{r}.tsv").unlink(missing_ok=True)
    ...
    def motif_acc(
        ...
        slurm = get_slurm_info()
        rank, world_size = slurm["rank"], slurm["world_size"]
        logger.info(f"[Rank {rank}/{world_size}] Starting motif_acc")
        ...
        df[seq_column] = df[seq_column].str.upper()
        ...
        if logits_path is not None:
            ...
            dataset = MultiMaskDataset(df_local[seq_column], tok, positions)
            loader = DataLoader(dataset, batch_size=batch_size, shuffle=False, num_workers=1)
            probs_local = _masked_probs(model_, tok, loader, dev, desc=f"[Rank {rank}] Masked logits motif_len={motif_len}")
    ...
    def core_noncore(
        ...
        slurm = get_slurm_info()
        rank, world_size = slurm["rank"], slurm["world_size"]
        logger.info(f"[Rank {rank}/{world_size}] Starting core_noncore")
        ...
        if logits_path is not None:
            ...
            dataset = MultiMaskDataset(df_local[seq_column], tok, positions)
            loader = DataLoader(dataset, batch_size=batch_size, shuffle=False, num_workers=1)
            probs_local = _masked_probs(model_, tok, loader, dev, desc=f"[Rank {rank}] Masked logits (core/non-core) motif_len={motif_len}")
            ...
            if world_size == 1:
                ...
                assert save_logits, "save_logits is required for multi-node runs"
                rank_path = Path(save_logits).with_suffix(f".rank{rank}.tsv")
                pd.DataFrame(probs_local, columns=["A", "C", "G", "T"]).to_csv(rank_path, sep="\t", index=False)
                logger.info(f"[Rank {rank}] Saved per-rank logits to {rank_path}")
