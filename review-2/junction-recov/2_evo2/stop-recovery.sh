...
# forward: upstream 4094 bp, predict stop codon (3 tokens)
srun python evo2_generate.py \
    --tsv ../results/${PREFIX}_stop_sites.tsv \
    --length 4094 \
    --output ../results/${PREFIX}_stop_sites_evo2_7b_ntokens_3 \
...
srun python evo2_generate.py \
    --tsv ../results/${PREFIX}_stop_sites.tsv \
    --length 4095 \
    --output ../results/${PREFIX}_stop_sites_evo2_7b_rc_ntokens_3 \
    --batch-size 2 --n-tokens 3 --reverse
